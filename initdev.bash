...
calculate_hashes() {
    find "$SRC_DIR" "$WEB_DIR" -type f -exec md5sum {} + > "$HASH_FILE.new"
}
...
minimize_console() {
    ...
    foreach (\$process in \$javaProcesses) {
        if (\$process.MainWindowHandle -ne [System.IntPtr]::Zero) {
            Write-Host \"Proceso de Java encontrado: \$(\$process.ProcessName) (ID: \$(\$process.Id))\"
            Write-Host \"Minimizando la ventana del proceso Java...\"
...
# Función para compilar y desplegar
compile_and_deploy() {
...
# Inicializa la primera compilación y despliegue
compile_and_deploy

# Monitorea cambios en los directorios src y web
while true; do
    calculate_hashes
